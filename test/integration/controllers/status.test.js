...
describe('GET /api/v1/employees/:employeeId/status', () => {
  ...
  describe('Error response', () => {
    it('should return error response if not logged in', async () => {
      const res = await request(app).get(`${url}/${employee._id}/status`);
...
describe('GET /api/v1/employees/:employeeId/status/:id', () => {
  let employee;
  let company;
  let token;
  let user;
...
describe('POST /api/v1/employees/:employeeId/status', () => {
  let employee;
  let company;
  let token;
  let user;
...
describe('PUT /api/v1/employees/:employeeId/status/:id', () => {
  let employee;
  let company;
  let token;
  let user;
...
describe('DELETE /api/v1/employees/:employeeId/status/:id', () => {
  let employee;
  let company;
  let token;
  let user;
  ...
  describe('Error response', () => {
    it('should return error response if not logged in', async () => {
      const res = await request(app).delete(
        `${url}/${employee._id}/status/${status._id}`,
      );
    ...
    it('should return error response if logged in user is not equal to company user', async () => {
      ...
      const res = await request(app)
        .delete(`${url}/${employee._id}/status/${status._id}`)
        .set({ 'x-company-tenant': company.slug })
        .auth(token, { type: 'bearer' });
    ...
    it('should return 404 if employeeId is invalid or not exist', async () => {
      ...
      const res = await request(app)
        .delete(`${url}/${employeeId}/status/${status._id}`)
        .set({ 'x-company-tenant': company.slug })
        .auth(token, { type: 'bearer' });
    ...
    it('should return 404 if status id param is invalid or not exist', async () => {
      ...
      const res = await request(app)
        .delete(`${url}/${employee._id}/status/${statusId}`)
        .set({ 'x-company-tenant': company.slug })
        .auth(token, { type: 'bearer' });
  ...
  describe('Success Response', () => {
    it('should return success response if ids are valid', async () => {
      ...
      expect(res.body).toEqual(
        expect.objectContaining({
          status: {},
          message: `Status - ID:${status._id} successfully deleted`,
        }),
      );
    });
  });
