...
describe('GET /api/v1/users/:id - getUser', () => {
  ...
  it('should return 404 is id is not found or invalid', async () => {
    ...
    res = await request(app).get(`${url}/${invalidObjectId}`).auth(token, { type: 'bearer' });
  ...
  it('should return 200 success reponse if id is valid', async () => {
    ...
    const res = await request(app).get(`${url}/${user._id}`).auth(token, { type: 'bearer' });
...
describe('PUT /api/v1/users/:id - updateUser', () => {
  ...
  describe('Success Response', () => {
    it('should return a success response if logged and an admin user', async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.success).toBeTruthy();
      expect(res.body).toEqual(expect.objectContaining({ message: 'Successfully updated' }));
      expect(res.body.user).toEqual(expect.objectContaining({ email: 'rodrigo@example.com' }));
    });
  });
});
